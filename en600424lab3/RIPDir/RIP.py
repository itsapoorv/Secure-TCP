...
class RIPTransport(StackingTransport):
    ...
    def sign(self, data):
        rawKey = CertFactory.getPrivateKeyForAddr(self.lowerTransport().getHost().host)
    ...
    def getHost(self):
        host = self.lowerTransport().getHost() 
        rawCerts = CertFactory.getCertsForAddr(host.host)
        certs = []
        for i in range(1, len(rawCerts)):
            certs.append(X509Certificate.loadPEM(rawCerts[i]))
        rawPrivateKey = CertFactory.getPrivateKeyForAddr(host.host)
        privateKey = RSA.importKey(rawPrivateKey)
        return PkiPlaygroundAddressPair.ConvertHostPair(host, certs, privateKey)

    def getPeer(self):
        peer = self.lowerTransport().getPeer() 
        certs = self.peerCerts
        return PkiPlaygroundAddressPair.ConvertPeerPair(peer, certs)
...
class RIPSuper():
    ...
    def setHost(self):
        self.host = self.higherTransport.lowerTransport().getHost().host
    ...
    def verify_certs(self, certs):
        certs.append(X509Certificate.loadPEM(CertFactory.getRootCert()))
...
class RIPClient(StackingProtocolMixin, Protocol, RIPSuper):
    ...
    def handshakeResponse(self, rMessage):
        ...
        self.higherTransport.peerCerts = certs
        ...
        if self.verify_certs(certs):
            if self.verifySignedMessage(rMessage):
                ...
                intNonce = intNonce + 1
                expectedNonce = hex(intNonce)[2:]
                if expectedNonce[-1] == "L":
                ...
                if rMessage.certificate[1] == expectedNonce:
                    ...
                    responseNonce = hex(responseNonce)[2:]
                    if responseNonce[-1] == "L":
                        responseNonce = responseNonce[:-1]
                    ...
                    self.higherTransport.writeSignedMessage(newRMessage, False)
                    self.higherTransport.nextSequenceNumber = self.higherTransport.nextSequenceNumber + 1
        ...
        for rMessage in self.storage.iterateMessages():
            if isinstance(rMessage, RIPMessage):
                ...
                if rMessage.sequence_number_notification_flag:
                    success = self.handshakeResponse(rMessage)
                    if success:
    ...
    def handshakeInitResponse(self, rMessage):
        ...
        if self.verify_certs(certs):
            if self.verifySignedMessage(rMessage):
                ...
                newRMessage.acknowledgement_flag = True
                newRMessage.acknowledgement_number = self.getAckNumber(rMessage)
                newRMessage.certificate = [newNonce, responseNonce] + certs
            ...
            if isinstance(rMessage, RIPMessage):
                ...
                if rMessage.sequence_number_notification_flag:
                    if self.higherTransport.state == "LISTEN":
                        success = self.handshakeInitResponse(rMessage)
                        if success:
                            self.expectPeerSeqNum = self.expectPeerSeqNum + 1
                            print "Handshake message 1 processed, ", int(rMessage.sequence_number), self.expectPeerSeqNum
                        continue
                    else:
                        if self.verifySignedMessage(rMessage):
                    ...
                    success = self.completeHandshake(rMessage)
                    if success:
                        self.expectPeerSeqNum = self.expectPeerSeqNum + 1
                        print "Handshake message 3 processed, ", int(rMessage.sequence_number), self.expectPeerSeqNum
                    continue
